...
#### 4.1.1 Virtual machine compute power isn't enough
...
The error thrown by the NexusDashboard application looks something like this: `sqlalchemy.exc.OperationalError: (pymysql.err.OperationalError) (1054, "Unknown column 'play_keys.times_used' in 'field list'")
[SQL: SELECT play_keys.id AS play_keys_id, play_keys.key_string AS play_keys_key_string, play_keys.key_uses AS play_keys_key_uses, play_keys.created_at AS play_keys_created_at, play_keys.active AS play_keys_active, play_keys.notes AS play_keys_notes, play_keys.times_used AS play_keys_times_used
FROM play_keys
WHERE play_keys.key_string = %(key_string_1)s
 LIMIT %(param_1)s]
`.
**Solved**: Looking at this error, the only important thing here is `"Unknown column 'play_keys.times_used' in 'field list'"`. To fix this, it is necessary to go into the MySQL database you setup and manually add the columns to the proper tables. This error occurred for the following tables: `accounts.email`, `accounts.email_confrirmed_at`, `accounts.active`, `accounts.name`, `play_keys.times_used`. Unfortunately, I wasn't actively recording each of these changes as I went along, so I am going off of the errors that I see in the NexusDashboard logs.
#### 4.1.5 A note before proceeding
These solutions are representative of a culmination of a few days troubleshooting a working, multiplayer server. It only encompasses the errors that I remember/had tremendous difficulty with. I'm sure, during your setup process, that you will encounter a few minor problems not listed in this tutorial. To fix these, I **HIGHLY** recommend that you use GPT as an aide. The solutions listed above use, in some small way or another, advice from the AI. **PLEASE**, before going into the LegoUniverse discord server, or messaging me, use your brain. Many of the problems you run into can be solved through intuition, trail and error, and patience.
